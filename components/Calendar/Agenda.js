...
class AgendaView extends React.Component {
  ...
  formatMonths = (dateFrom, dateTill, monthsAlreadyUsed, colors) => {
    ...
    monthsAlreadyUsed.push(currentMonth);
  ...
  const subtitleText = {
      ...
      fontSize: 12, padding: 0, margin: 0, marginLeft: 4
  }
  ...
  onViewableItemsChanged = ({ viewableItems, changed }) => {
    this.props.changeHeaderTitle(viewableItems[0].item.dateFrom);
  };
...
class Event extends React.Component {
  ...
  filterData = data => {
    ...
    */
    ...
    let filteredData = data
      .filter(item => {
        return isFuture(parse(item.dateTill));
      })
      .sort((a, b) => {
  ...
  render() {
    ...
    return (
      <View style={{ flex: 1, backgroundColor: this.props.colors.surface }} onLayout={() => {this.setState({ animatedLayout: true })}}>
        {data.length > 0 ? (
          <AgendaView
            darkTheme={this.props.darkTheme}
            sortedData={data}
...
export default class Agenda extends React.Component {
  ...
  Tparse = () => {
    const nyTimeZone = "Europe/Tiraspol";
    const date = new Date("2019-10-14T02:02:20Z");

    const nyDate = utcToZonedTime(date, nyTimeZone);
    let NY = format(date, "yyyy-MM-dd HH:mm:ssXXX", {
