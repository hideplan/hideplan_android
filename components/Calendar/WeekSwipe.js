...
import dateFns, { isFuture, getHours, getDay, getYear, getMonth, getDate, isSameDay, areIntervalsOverlapping, differenceInMinutes, addDays, subDays, differenceInCalendarDays, isToday } from "date-fns";
  ...
  checkOverlappingHeaderEvents = (firstDate, secondDate) => {
    ...
    if (dataset) {
      ...
      if (differenceInCalendarDays(parse(event.dateTill), parse(event.dateFrom)) > 0 || event.allDay == "true") {
        ...
        dataset.map(item2 => {
          if (this.checkOverlappingEvents(event, item2) && event.uuid !== item2.uuid && differenceInCalendarDays(parse(item2.dateTill), parse(item2.dateFrom)) > 0 && offsetCount.includes(item2.uuid) == false || this.checkOverlappingEvents(event, item2) && event.uuid !== item2.uuid && offsetCount.includes(item2.uuid) == false && item2.allDay == "true") {
            width = width + 1 //add width for every overlapping item
            offsetCount.push(item2.uuid)
          } else if (this.checkOverlappingEvents(event, item2) && event.uuid == item2.uuid && differenceInCalendarDays(parse(item2.dateTill), parse(item2.dateFrom)) > 0 && offsetCount.includes(event.uuid) == false || this.checkOverlappingEvents(event, item2) && event.uuid == item2.uuid && item2.allDay == "true" && offsetCount.includes(event.uuid) == false) {
...
const style = {
  ...
  left: this.props.offsetLeft, opacity: isFuture(parse(this.props.event.dateTill)) ? 1 : 0.6,
}
...
render() {
  ...
  return (
    <View style={{ flex: 1, width: WIDTH }}>
          ...
          onLayout={() => {this.setState({ animatedLayout: true })}}>
         <View 
    ...
    : null
              ...
              <ScrollView
                ...
                <View style={{ flex: 1,                      width: WIDTH,
                  ...
                  ><View style={{ flex: 1, position: "absolute", height: 40, top: this.state.showNewEvent.top, width: (WIDTH - 30) /7 , left: this.state.showNewEvent.left, backgroundColor:       this.props.colors.primary,  borderWidth: 1, borderColor: this.props.colors.primary,  alignItems: "center", borderRadius: 4, zIndex: 9999}}>
...
class FilteredView extends React.Component {
  ...
  filteredEvents = (week) => {
    ...
    let data = {events: weekEvents, eventsHeader: weekEventsHeader, hasHeaderEvents: hasHeaderEvents}
  ...
  <View style={{ width: WIDTH-30, padding: 0, margin: 0, flexDirection: "column"}}>
         <View 
         style={{ height: 40, width: WIDTH, padding: 0, margin: 0, flexDirection: "column", backgroundColor: this.props.colors.header,
        }} onLayout={() => {
...
export default class WeekSwipe extends React.Component {
  ...
  getDaysInWeek = (date) => {
    ...
    for (let i=1; i<8; i++) {
      week2.push(addDays(parse(week1[6]), i))
    }
    weekDays.push(week0)
    weekDays.push(week1)
    weekDays.push(week2)
  ...
  render () {
    ...
    return (
      <View style={eventListStyle}>
        ...
        {this.props.allData && !this.state.isLoadingCalendar
        ?
         <FilteredView 
         ref="FilteredView"
         darkTheme={this.props.darkTheme}
