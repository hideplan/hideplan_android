...
class ListInput extends React.Component {
  constructor(props) {
    ...
    this.state = {
      calendar: "",
      color: {"name": "indigo", "s200": "#9fa8da", "s600": "#3949ab"},
      colorsVisible: false
    };
  }
  ...
  saveCalendar = () => {
    ...
    calendarObj = JSON.stringify({uuid: uuid, calendar: this.state.calendar, color: color, isChecked: true, updated: timestamp.toString()})
    ...
    renderColors = (colorList) => {
      return colorList.map(color => {
        return <TouchableNativeFeedback onPress={() => {this.selectColor(color), this.MyModal.close()}} background={this.props.darkTheme ? TouchableNativeFeedback.Ripple(
        ...
        <View style={{flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "flex-start", width: "100%", padding: 14}}>
          ...
          <Text style={{color: this.props.colors.gray, fontSize: 16 }}> 
            {color.name}
          </Text>
    ...
    render() {
      ...
      const floatColorInput = {
        ...
        height: 30,
        borderRadius: 15,
        backgroundColor: this.props.darkTheme ? this.state.color.s200 : this.state.color.s600,
        flex: 1,
...
onPress={() => this.props.navigation.goBack(null)}
 /> }}
icons={[ <Button uppercase={false} mode="text" color={this.props.screenProps.colors.primary} onPress={() => this.child.current.saveCalendar()}>Save</Button>]}
/>
